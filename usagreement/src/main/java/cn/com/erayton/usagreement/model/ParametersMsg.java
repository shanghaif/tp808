 ...
 * 平台下发的设置参数
 * */
public class ParametersMsg extends PacketData {
    private String TAG = ParametersMsg.class.getName() ;
    ...
    public void inflatePackageBody(byte[] data) {
        ...
        for (int i=0; i< parameterList.length; i = i+4){
            Log.d(TAG, "inflatePackageBody ----------- "+getParameterSettingsString(bitOperator.parseIntFromBytes(parameterList, i, 4))+" ------------------------ "+i+" --- "+ bitOperator.parseIntFromBytes(parameterList,i,4)) ;
