package cn.rayton.camera2.manager;
...
import cn.rayton.camera2.utils.CameraUtil;
...
public class VideoSession extends Session {
    ...
    private void createVideoSession(int deviceRotation) {
        ...
        try {
            ...
            Log.e("cjh", "-createVideoSession------------------------------------"+deviceRotation) ;
            e.printStackTrace();
        }
    }
    ...
    private void setUpMediaRecorder(final int deviceRotation) {
        if (mMediaRecorder == null) {
...
//        //  设置存储时间长度
        mMediaRecorder.setMaxDuration(10000);      //  3600000-> ms -> 3600 s -> 60 min -> 1 hour
        int rotation = CameraUtil.getJpgRotation(characteristics, deviceRotation);
        mMediaRecorder.setOrientationHint(rotation);
        mMediaRecorder.setOnInfoListener(new MediaRecorder.OnInfoListener() {
            @Override
            public void onInfo(MediaRecorder mr, int what, int extra) {
                Log.e("cjh", "onInfo ------------------"+what) ;
//                if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_DURATION_REACHED){
//                    BusManager.getEvents().postRecordStatus(new RecordEvent(1));
//
//                    Log.e("cjh", "MEDIA_RECORDER_INFO_MAX_DURATION_REACHED ------------------") ;
//                }
            }
        });
        try {
