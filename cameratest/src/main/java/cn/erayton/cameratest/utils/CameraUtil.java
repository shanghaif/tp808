...
public class CameraUtil {
    ...
    public static Size getPreviewSizeByRatio(StreamConfigurationMap map, Point displaySize, double ratio){
        Size[] supprotSize = map.getOutputSizes(SurfaceTexture.class) ;
        sortCamera2Size(supprotSize);
        for (Size size : supprotSize){
            boolean isRatioMatch = (size.getHeight()* ratio) == size.getWidth() ;
            if (!isRatioMatch){
                continue;
            }
            if ((size.getHeight() == displaySize.x)
