...
public class IndicatorView extends View {
    ...
    private float mClickDistence ;
    private float mMaxDistence ;
    ...
    public IndicatorView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mClickDistence = mDisplayPoint.x / 20 ;
        mMaxDistence = mDisplayPoint.x / 10 ;
        // init animation
    ...
    public void setIndicatorListener(IndicatorListener mListener) {
    ...
    private ValueAnimator.AnimatorUpdateListener mUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            mCurrentTranslate = (int) animation.getAnimatedValue();
            postInvalidate();
        }
    } ;
    ...
    private void detectGesture(float downX, float upX, float downY, float upY) {
        ...
        if (Math.abs(distanceX) < mClickDistence
            && Math.abs(distanceY) < mClickDistence
            && mTouchTime < DELAY_TIME){
            // click event, change index
            checkAndChangePosition(calcClickIndex(upX), true);
        }

        if (Math.abs(distanceX) > mMaxDistence){
